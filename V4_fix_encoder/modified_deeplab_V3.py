...
def DeepLabV3Plus(img_height, img_width, nclasses=66):
    ...
    x_a = Upsample(tensor=x_a, size=[img_height // 4, img_width // 4])  # 여기다가 지금 보고있는 논문의 dilated conv를 추가해볼까?
    ...
    first_output = Upsample(x, [img_height, img_width])
    ...
    '''     
    model = Model(inputs=base_model.input, outputs=[x, first_output, second_output], name='DeepLabV3_Plus')
    model.load_weights("/yuhwan/yuhwan/Projects/[6]My_sub_experiment/Segmentation/V1/last_epoch.h5")
